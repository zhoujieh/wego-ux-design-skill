...
职责链：需求确认 → 设计决策（产物驱动） → 实现 → 验证与输出。每阶段未通过门禁不得进入下一阶段。
...
执行 `04-ai-rules/08-requirement-confirmation-rules.md`，确认三个维度：

- 用户任务：用户要完成什么（任务描述，不是功能描述）
- 业务目标：做成什么样算成功（一个优先级最高的目标）
- 使用场景：谁在什么情况下用（具体的用户和情境）

**门禁：** AI 输出"我理解这个页面是：[谁] 在 [什么情境] 下，要 [完成什么任务]，业务目标是 [什么]。对吗？" → 用户确认通过；用户修正 → 针对修正维度重新确认。确认语句必须输出，不得跳过。

**简短模式：** 用户要求"直接输出"时，仍需输出确认语句，但不需要等待用户确认即可进入阶段二。不得跳过确认语句直接进入阶段二。
