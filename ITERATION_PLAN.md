...
- [ ] **4.1.4** 创建 `design-library/components/tabs.json` — ⚠️ 待样式优化
...
> **⚠️ 注意**：4.1.4~4.1.5, 4.1.7~4.1.13 共 11 个组件需先完成样式优化（硬编码清理、Token 对齐）后再编写 JSON 契约。actionsheet、navbar 组件已完成优化，可作为模板参照。
...
- [x] **4.2.1** 创建 `design-library/components/index.json`
  ...
  - 已注册 13 个组件：4 个 stable（actionsheet、button、link、navbar），9 个 optimizing（待样式优化）
