...
`rules/design-principles.md` 是兜底安全网：当组件 `doNotInvent` 和 `hardConstraints` 还没覆盖某个场景时，AI 至少有一个方向判断依据。当以下条件全部满足后可删除：
...
- [x] **3.3.2** `actionsheet` — 创建 `preview/component-actionsheet.html` + Canonical CSS
  ...
  - ✅ 硬编码已清除：separator 背景色已 Token 化；safe-area block-size 已用 `env(safe-area-inset-bottom, 34px)`；仅 `::after` 的 134px/5px 为 iOS Home Indicator 平台约定值
...
- [x] **4.1.1** 创建 `design-library/components/button.json` ✅ 已完成（含 icon 场景优化）
- [x] **4.1.2** 创建 `design-library/components/link.json` ✅ 已完成
- [x] **4.1.3** 创建 `design-library/components/navbar.json` ✅ 已完成
- [x] **4.1.4** 创建 `design-library/components/tabs.json` ✅ 已完成
- [x] **4.1.5** 创建 `design-library/components/dialog.json` ✅ 已完成（含样式优化）
- [x] **4.1.6** 创建 `design-library/components/actionsheet.json` ✅ 已完成
- [x] **4.1.7** 创建 `design-library/components/toast.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.8** 创建 `design-library/components/loading.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.9** 创建 `design-library/components/input.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.10** 创建 `design-library/components/form.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.11** 创建 `design-library/components/tag.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.12** 创建 `design-library/components/cell.json` ✅ 已完成（样式问题先记录到 `knownIssues`）
- [x] **4.1.13** 创建 `design-library/components/result.json` ✅ 已完成（样式问题先记录到 `knownIssues`）

> **当前执行策略**：阶段 4 先补齐 JSON 契约与注册表主流程，组件样式问题不阻塞契约产出；所有已识别的样式问题统一记录到各组件契约的 `knownIssues`，后续再专项处理硬编码清理和 Token 对齐。
...
- [x] **4.2.1** 创建 `design-library/components/index.json`
  - 列出全部组件的 slug、name、preview 路径
  - 已注册 13 个组件：5 个 stable（actionsheet、button、link、navbar、tabs），8 个 optimizing（待样式优化）
...
- [x] **5.1.1** 创建 `design-library/library-consumption.json`
...
- [x] **5.2.1** 在 `SKILL.md` 顶部增加「设计库路径」区块
  - 明确指出 `design-library/` 路径
  - 明确指出 AI 必须先读 `design-library/library-consumption.json`

- [x] **5.2.2** 在 `rules/execution.md`（原 `04-ai-rules/01-ai-execution-rules.md`）中，阶段三实现阶段增加设计库消费子步骤：
...
- [x] **5.2.3** 移除旧的 `02-tokens/` 目录下的冗余 md 文件（Token 使用指南合并到 `rules/tokens.md`，架构说明不再需要）
  - 保留 `02-tokens/tokens.json` 作为 Token 定义的原始来源（供 `generate_tokens.py` 使用），其他的归档或删除
  - 保留 `02-tokens/tokens.css` 作为生成物引用

- [x] **5.2.4** 移除旧的 `03-components/` 目录下的组件 md 文件（被 `design-library/components/*.json` + `preview/*.html` 替代）
...
- [x] **6.1.1** 在 `scripts/validate_components.py` 中增加检查项：
...
- [x] **6.2.1** 创建 `scripts/validate_library.py`，检查项：
...
- [x] **6.3.1** 验证 `design-library/tokens.css` 中的 Token 定义与 `02-tokens/tokens.json` 数据源一致
...
- [x] **7.1.1** 测试 Prompt：「在微购设计系统中生成一个 strong 主按钮」
  ...
  - 验收：AI 输出中包含 `class="wg-button wg-button--page wg-button--strong"` 且无硬编码样式
  - 结果：`tests/fixtures/generated/主按钮` 通过 `scripts/validate_generated_projects.py --require-fixtures`

- [x] **7.1.2** 测试 Prompt：「在微购设计系统中生成一个带标题和操作的 dialog」
  - 预期 AI 行为：类似上述流程，但针对 dialog 组件
  - 验收：AI 输出中包含 dialog 的 `.wg-dialog` class，结构正确
  - 结果：`tests/fixtures/generated/弹窗` 包含 `.wg-dialog`、`.wg-dialog__title`、`.wg-dialog__buttons`，并通过生成项目校验

### 7.2 AI 整页生成测试

- [x] **7.2.1** 测试 Prompt：「生成一个微购店铺设置页，包含导航栏、表单行、保存按钮」
  - 预期 AI 行为：按 `library-consumption.json` 顺序读取 → 组合 navbar + form + button 组件 → 自己写外层布局 grid
  - 验收：
    - [x] 所有设计值使用 `var(--wg-*)`
    - [x] 组件 class 名与 Canonical CSS 一致
    - [x] 页面无硬编码颜色/间距
    - [x] 交互状态完整（hover/active/disabled）
  - 结果：`tests/fixtures/generated/店铺设置` 组合 `.wg-navbar`、`.wg-form`、`.wg-button wg-button--page wg-button--strong`，并通过生成项目校验

### 7.3 回归测试

- [x] **7.3.1** 用 `tests/golden-prompts/` 中的已有黄金用例重新生成，对比新旧输出
  - 检查新输出是否有任何硬编码值
  - 检查新输出的组件 class 是否正确
  - 验收：新输出质量不低于旧输出，且无硬编码
  - 结果：`登录`、`商品发布`、`订单列表`、`搜索空状态`、`删除确认`、`表单校验失败`、`结果页`、`多页导航` 8 个 golden prompt fixture 已生成并通过校验
...
| 旧路径 | 新路径 | 说明 |
|--------|--------|------|
| `01-principles/01-design-principles.md` | `rules/design-principles.md` | 迁移到 rules/ 并去数字前缀 |
...
| 决策 | 说明 |
|------|------|
| 三层架构 | 控制层（rules/principles）→ 产物层（design-library）→ 工具层（scripts），单向依赖 |
