...
| 页面类型 | 宽度策略 | 说明 |
|---|---|---|
| 表单型页面（登录、注册、设置、重置等） | 自适应容器宽度 | 给 `wg-button-group` 加 `--form` 修饰符，按钮宽度跟随内容区宽度（`inline-size: 100%`） |
| 结果型页面（操作成功、操作失败等） | 固定宽度 `wg.component.button.width.page.single` | 居中展示，不需要撑满 |
| 浏览型页面底部的固定操作栏 | 自适应容器宽度 | 给 `wg-button-group` 加 `--form` 修饰符，跟随操作栏宽度 |

判断流程：

```text
页面是否为表单型？
├─ 是 → 给 wg-button-group 加 --form 修饰符（自适应容器宽度）
└─ 否 → 页面是否为结果型？
...
实现方式：
- 表单型/浏览型底部操作栏：`class="wg-button-group wg-button-group--page wg-button-group--single wg-button-group--form"`
- 结果型：`class="wg-button-group wg-button-group--page wg-button-group--single"`
- 禁止在 app.css 中覆盖 `.wg-button` 的 `inline-size` — 宽度策略通过 Button Group 修饰符表达
...
- 页面级规则：同一页面中，同一业务含义的操作只能有一个 strong / primary button。如果同一动作出现在多个区域（NavBar、底部操作区、内容区），其它入口必须降级为 secondary / text / weak。组件使用者必须基于业务动作去重，而不是基于视觉区域去重。
...
/* ── 表单型宽度 ── */
.wg-button-group--form {
  inline-size: 100%;
}

.wg-button-group--form .wg-button {
  inline-size: 100%;
}
...
- `default`：正常可用
- `hover`：仅桌面原型预览，不作为移动端必要状态
- `active / pressed`：必须可见，只改变操作背景，不位移、不缩放
- `disabled`：必须降低视觉权重并禁用点击，使用 `disabled` 属性
- `loading`：V1 未定义视觉变体，但提交类操作必须防止重复提交——通过 `disabled` + `aria-busy="true"` + 页面级文案实现

禁止只有 default 和 hover 而没有 pressed 状态。
...
- 不得覆盖背景色、文字色、圆角、高度和 padding。
- 按钮宽度通过 Button Group 的宽度修饰符（`--form`）控制，不得在 app.css 中直接覆盖 `.wg-button` 的 `inline-size`。
- 不得使用内联 style 修改组件。
...
- 禁止同一业务动作在页面中出现多个 strong 按钮。
- 禁止 Dialog 主按钮、页面底部按钮、NavBar 按钮同时表达同一核心动作。
- 禁止重复提交按钮都使用主按钮。
