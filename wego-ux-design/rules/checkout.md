...
> Version 3.0
> 本文档定义 AI 输出前必须执行的最终检查清单。对照阶段二《设计库消费计划》验证，而非泛泛检查。
...
□ openMode / layoutMode / background / maxWidth / 固定区是否落地
...
□ 组件 markup 是否从 preview 复制，未改写 class、modifier、DOM anatomy
□ 命中组件的 Canonical CSS 是否逐字复制写入 styles/components.css
□ 组件状态和变体是否来自契约，未自创组件变体
...
□ 样式加载顺序是否为 tokens.css → iconfont.css → components.css → app.css
