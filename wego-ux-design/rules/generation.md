...
所有可滚动区域必须隐藏浏览器默认滚动条，保持统一的视觉一致性。
...
当前阶段生成可交互 Web 原型项目。正式组件契约只来自 `design-library/components/*.json`，不得放入或引用 KuiklyUI API。

组件选择流程：

```text
读取 `design-library/components/index.json`
↓
根据用户目标匹配主流程组件
↓
扫描页面中是否存在辅助交互元素（链接、提示、空状态等）
↓
将辅助元素匹配到组件注册表中对应组件（如 <a> → Link、空态提示 → Result）
↓
读取 `rules/components.md` 和所有被选中组件的 JSON 契约与 preview 文件
↓
先确定每个组件的语义维度
↓
