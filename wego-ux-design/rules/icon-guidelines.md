...
- **CSS 前缀**：`icon-`
- **权威来源**：`design-library/assets/fonts/iconfont/iconfont.json`
- **字体文件**：`design-library/assets/fonts/iconfont/iconfont.css` + 对应字体文件（woff2 / woff / ttf）

> **路径说明**：`design-library/assets/fonts/iconfont/iconfont.json` 是运行时查询源（AI 生成页面时使用），`website/assets/fonts/iconfont/iconfont.json` 是 website 本地副本——两者内容一致，运行时只需读取 `design-library/assets/fonts/iconfont/` 下的文件即可。
...
生成界面时，必须先读取 `design-library/assets/fonts/iconfont/iconfont.json`，在 `glyphs` 数组中查找所需图标的 `font_class`。只有确认 iconfont 中不存在所需图标时，才使用 SVG 兜底方案。
...
归属位置：design-library/assets/fonts/iconfont/
```
