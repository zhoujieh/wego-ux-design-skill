...
> Version 3.0
> 定义 AI 执行微购设计任务的阶段门禁、设计库消费计划、设计库消费顺序、验证与输出要求。
...
- 缺失信息只影响局部细节时，可列为“设计假设”，但必须在确认卡中显式标注并请求确认。
...
阶段二只产出阶段三消费设计库必须的信息。全部内部完成，不展示给用户。设计判断细节按 `rules/generation.md` 执行，但不作为本文档的长链路门禁。先理解页面目标和结构，再选择组件；禁止从组件开始拼页面。

内部产物固定为《设计库消费计划》：

```text
设计库消费计划
- 页面粗类型：[浏览 / 操作 / 表单 / 结果 / 异常 / 空]
- ui_kit：[命中 type 或 未命中]；约束来源：[quality-report.json 路径或通用规则]
- 页面壳约束：[openMode / layoutMode / background / maxWidth / 固定区]
...
- 命中 `ui_kit` 时，必须读取对应 `quality-report.json`，将 `openMode` / `layoutMode` / `background` / `layoutRulesCaptured` 写入计划并逐条遵守。
- NavBar 左侧为取消或关闭时，`openMode` 必须为 `present-bottom`，不得被页面粗类型推理覆盖。
- 未命中 `ui_kit` 时，按 `generation.md` §6.8 通用规则处理，并按 §4 标注缺失。
- 组件消费必须从 `components/index.json` 选择 slug；不得扫描 `preview/` 或凭视觉相似猜组件。
- 每个命中组件必须写明契约文件和 preview 文件；状态和变体只能来自组件契约。
...
↓
recommendedReadOrder: tokens.json / tokens.css / scaffold.css / components/index.json
↓
components/{slug}.json（阶段二《设计库消费计划》命中的每个组件）
↓
preview/component-{slug}.html
↓
components.css
↓
assets/fonts/iconfont/（组件需要图标时）
```

读取与复制规则：

- 必须先读 `design-library/library-consumption.json`，获取 `recommendedReadOrder`、`consumptionLayers`、`downstreamScenarios`、`hardConstraints`。
- 不得未读消费契约就直接读组件预览或开始写代码。
- 不得先扫 `preview/` 再回头猜 slug；不得跳过组件契约直接复制结构。
- 复制 `design-library/tokens.css` → 项目 `styles/tokens.css`。
- 需要基础排版和布局工具时，按需复用 `design-library/scaffold.css` 中的通用骨架规则。
- 复制 `design-library/components.css` → 项目 `styles/components.css`。
- 使用图标时，复制 `design-library/assets/fonts/iconfont/` 整个目录 → `assets/fonts/iconfont/`，并按 `rules/icon-guidelines.md` 处理 iconfont 缺失场景。
- 从 `preview/component-{slug}.html` 只复制组件 markup，不复制演示壳、矩阵或说明文案。
- `components/{slug}.json`、`components/index.json` 只用于理解契约，不是可直接输出给用户的页面。

主体实现规则：

- HTML：语义结构 + 资源引用；样式加载顺序为 `tokens.css` → `iconfont.css`（如有）→ `components.css` → `app.css`。
...
- 未通过项必须修复后重新验证；浏览器验证或部署若未满足适用条件，必须以“降级”或“阻断”状态明确记录，不得省略。
